...
.modal, .rename_modal, .delete_modal {
...
/* Modal Content */
.modal-content, .rename_modal-content, .delete_modal-content {
	position: relative;
...
/* The Close Button */
.close, .rename_close, .delete_close {
	color: white;
	float: right;
	font-size: 28px;
	font-weight: bold;
...
.close:hover, .close:focus, .rename_close:hover, .rename_close:focus,
...
.modal-header, .rename_modal-header, .delete_modal-header {
	padding: 2px 16px;
	background-color: teal;
	color: white;
}

.modal-body, .rename_modal-body, .delete_modal-body {
	padding: 2px 16px;
}

.modal-footer, .rename_modal-footer, .delete_modal-footer {
	padding: 2px 16px;
	background-color: teal;
	color: white;
}
...
<div class="content">
	<%
		...
		String tag_filter = (String) request.getAttribute("tag_filter_his");
		session.setAttribute("tag_filter_his", tag_filter);
		String special_tag = (String) request.getAttribute("special_tag_his");
		session.setAttribute("special_tag_his", special_tag);
		ArrayList<String> all_tags = (ArrayList<String>) session.getAttribute("all_tags_his");
		ArrayList<String> all_languages = (ArrayList<String>) session.getAttribute("all_languages_his");
		String listOwner = (String) request.getAttribute("listOwner");
	%>
	<!-- Sidebar/menu -->
	...
	<div class="w3-main" style="margin-left: 250px; padding-top: 38px">
		...
		<section class="w3-row-padding w3-center">
			...
			<%-- <div style="text-align: left; padding-left: 15px">
				<i class="fa fa-user "></i>&nbsp;<%=listOwner%></div> --%>
			<form action="GetAllLists" method="post" style="text-align: left; padding-left: 25px">
				<input type="hidden" name="listOwner" value="<%=listOwner%>">
				<button type="submit" class="w3-button">
					<i class="fa fa-user"></i>&nbsp;<%=listOwner%>
				</button>
			</form>
			...
			<%
				ArrayList<UserList> all_lists = (ArrayList<UserList>) session.getAttribute("all_lists_his");
				session.setAttribute("current_lists_his", all_lists);
				String listname = "";
				String lang1 = "";
				String lang2 = "";
				String tag = "";
			...
			<div class="box">
				...
				<div class="box2">
					...
					<form action="GetWords.do" method="post">
						...
						<button type="submit">
							<img src="list4.png" alt="Random Name"
								style="width: 70%; padding-bottom: 10px;cursor:pointer;">
						</button>
					</form>
					<%-- <div class="small_tags" style="overflow: auto">#<%=lang1%>-<%=lang2%></div> --%>
					...
					<%-- <div class="small_tags" style="overflow: auto">#<%=tag%></div> --%>
	...
	<script>
		...
		function delete_click(clicked_id, listname) {
			document.getElementById('delete_warning').innerHTML = 'The list : <b>'
					+ listname + '</b> will be deleted';
